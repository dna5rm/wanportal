#!/usr/bin/env perl
# agent_image_pkgs.t - Dockerfile apk packages cover netping-agent.pl use lines
#
...
#      actually present in the Dockerfile `apk add --no-cache` line
...
#      is mapped below and shipped in Dockerfile.
#
# Skips when Dockerfile or netping-agent.pl is missing (image removed
...
unless (-f "$root/agent/Dockerfile" && -f "$root/agent/netping-agent.pl") {
    plan skip_all => 'agent/Dockerfile or agent/netping-agent.pl not present';
    exit 0;
}
...
my $df    = slurp("$root/agent/Dockerfile");
...
if (!@apk_lists) {
    fail('Dockerfile has an apk add --no-cache line');
    done_testing();
    exit 0;
}
...
for my $mod (sort @external) {
    ok($PKG{$mod}, "netping-agent.pl: `use $mod` is mapped to an apk package"
        . ' (add it to %PKG here and to Dockerfile)');

    if ($PKG{$mod}) {
        ok($shipped{ $PKG{$mod} },
            "Dockerfile ships $PKG{$mod} for $mod");
    }
}

# 2. HTTPS support for LWP has no `use` line; pin it anyway.
ok($shipped{'perl-lwp-protocol-https'},
    'Dockerfile ships perl-lwp-protocol-https (https support for LWP)');

# 3. Container basics: crond is the main process, tzdata for log timestamps.
ok($shipped{'cronie'}, 'Dockerfile ships cronie (crond main process)');
ok($shipped{'tzdata'}, 'Dockerfile ships tzdata (log timestamps)');
...
for my $pkg (sort keys %EXCLUDED) {
    ok(!$shipped{$pkg}, "Dockerfile does not ship $pkg");
