...
- `tests/validate.sh`: `perl -c` on `cgi-bin/api`, `php -l` on every `htdocs` page, a live pass over `GET /health` plus an admin login (the password is read from the container env, never hardcoded), a URL-contract pass (`/` serves the SPA shell, `/classic` and `/classic/login.php` serve the PHP console, `/app` is 404), grep gates for fixed audit regressions (LDAP filter escaping, /rrd path allowlist, credential password stripping, delete path prefixes, escaped page titles, cron wrapper privileges), then `tests/run.sh` as the last section. Exit 0 only when everything passes.
