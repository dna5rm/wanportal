 ...
 * row edits still pointed at the classic console, the bar's New opening
 * the in-app create form. The fetch
...
describe('MonitorsView', () => {
    it('fetches the listing once and renders the rows problems-first', async () => {
        ...
        // Row editing stays on the classic console; the bar's New opens
        // the in-app create form.
        expect(rows[2].find('td:last-child a').attributes('href')).toBe('/monitors_edit.php?id=' + M7)
        expect(rows[0].find('td:last-child a').attributes('href')).toBe('/monitors_edit.php?id=' + M9)
